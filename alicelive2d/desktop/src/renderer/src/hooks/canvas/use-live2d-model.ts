...
export const useLive2DModel = ({
  ...
  const handleMouseUp = useCallback((e: React.MouseEvent) => {
    const maybeTriggerFlirtyReply = () => {
      ...
      const teaseLine = pickRandomTeaseLine();
      LAppDelegate.getInstance().showTouchDebug(`[TEASE] ${teaseLine.text}`);
      reportLive2DDragDebug("trigger_fired", {
        teaseId: teaseLine.id,
        text: teaseLine.text,
        audioUrl: teaseLine.audioUrl,
        cooldownMs: FLIRT_TRIGGER_COOLDOWN_MS,
      });
      appendTeaseMessage(teaseLine.text);
      playTeaseAudio({ audioUrl: teaseLine.audioUrl, blockWhenBusy: true });
    };
    const adapter = (window as any).getLAppAdapter?.();
    const model = adapter?.getModel();
    const view = LAppDelegate.getInstance().getView();
...
Live2DDebug.playRandomMotion("")  // Play random motion from default group
    ...
    (window as any).triggerLive2DTeaseMessage = (_message: string) => {
      ...
      const matched = FLIRT_TRIGGER_LINES.find((item) => item.text === _message) || null;
      appendTeaseMessage(_message);
      if (matched) {
        playTeaseAudio({ audioUrl: matched.audioUrl, blockWhenBusy: true });
      }
      return true;
    };
