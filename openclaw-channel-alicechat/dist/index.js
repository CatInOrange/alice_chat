...
function auditFrame(stream, direction, frame, meta = {}) {
    ...
    try {
        ...
        fsSync.appendFileSync(path.join(FRAME_AUDIT_DIR, `${day}.jsonl`), `${JSON.stringify(record)}\n`, 'utf8');
    }
    catch (error) {
...
function materializeInboundMediaList(attachments) {
    ...
    for (const att of Array.isArray(attachments) ? attachments : []) {
        ...
        if (url.startsWith('/'))
            list.push({ path: url, contentType: mimeType || undefined });
    }
    return list;
}
...
function registerClient(activeClients, client) {
    ...
    for (const key of keys)
        activeClients.set(String(key), client);
    client._keys = [...keys];
}
...
function summarizeTodoSnapshot(snapshot, options = {}) {
    ...
    let filteredTasks = tasks.filter((task) => {
        ...
        if (!includeArchived && project?.archived)
            return false;
        if (projectId && String(task.projectId || '') !== projectId)
            return false;
        if (scope === 'completed')
...
function createBridgeServer(ctx) {
    ...
    if (!channelRuntime)
        throw new Error('channelRuntime unavailable; OpenClaw version too old');
    ...
    async function processChatRequest(ws, frame, cfg) {
        ...
        const sendBridgeFrame = (outFrame, phase) => {
            ...
            if (frameType === 'chat.reply_final')
                replyFinalSent = true;
            if (frameType === 'chat.run_final')
                runFinalSent = true;
...
const alicechatPlugin = {
    ...
    outbound: {
        ...
        sendMedia: async (ctx) => {
            ...
            if (ctx.mediaUrl)
                mediaCandidates.push(ctx.mediaUrl);
            if (Array.isArray(ctx.mediaUrls))
                mediaCandidates.push(...ctx.mediaUrls);
    ...
    gateway: {
        startAccount: async (ctx, _account) => {
    ...
    api.registerTool({
        ...
        async execute(_id, params) {
            try {
                ...
                switch (actionType) {
                    case 'play_track': {
                        const normalizedTrack = normalizeMusicTrack(params.track, requestId || 'music-action:play-track', 0, params.track?.artworkTone || 'aurora');
                    ...
                    case 'play_playlist': {
                        const normalizedDraft = normalizeMusicPlaylistDraft(params.playlistDraft, requestId || 'music-action:play-playlist');
                        const normalizedPlaylist = normalizeMusicPlaylistRef(params.playlist || playlistDraftToPlaylistRef(normalizedDraft), normalizedDraft?.tracks?.length || 0);
                    ...
                    case 'queue_next':
                    case 'queue_append': {
                        const normalizedTracks = normalizeMusicTracks(params.tracks || (params.track ? [params.track] : []), requestId || `music-action:${actionType}`, params.artworkTone || 'aurora');
                    ...
                    case 'save_ai_playlist': {
                        const normalizedDraft = normalizeMusicPlaylistDraft(playlistDraftInput, requestId || params.id || `ai-playlist:${Date.now()}`);
        ...
        async execute(_id, params) {
            try {
                ...
                if (String(input.projectId || '').trim())
                    payload.projectId = String(input.projectId).trim();
                if (String(input.projectName || '').trim())
                    payload.projectName = String(input.projectName).trim();
                ...
                if (input.subtasks !== undefined) {
                    payload.subtasks = normalizeTodoSubtasks(input.subtasks, requestId || actionType || 'todo-action');
                }
