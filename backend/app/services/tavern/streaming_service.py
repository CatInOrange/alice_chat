...
class TavernStreamingService:
    ...
    def send(self, chat_id: str, *, text: str, preset_id: str = '', provider_id: str = '', instruction_mode: str = '', suppress_user_message: bool = False) -> dict[str, Any]:
        hidden_instruction = self.tavern_service._hidden_instruction_text(instruction_mode)
        prepared = self.tavern_service.prepare_generation(chat_id, text=text, preset_id=preset_id, hidden_instruction=hidden_instruction)
        ...
        user_message = None if suppress_user_message else self.tavern_service.store.append_message(chat_id, role='user', content=text)
        ...
        try:
            ...
            logger.warning('Tavern summary post-processing failed for chat %s: %s', chat_id, exc, exc_info=True)
        ...
        try:
            ...
            logger.warning('Tavern worldbook runtime post-processing failed for chat %s: %s', chat_id, exc, exc_info=True)
    ...
    def stream(self, chat_id: str, *, text: str, preset_id: str = '', provider_id: str = '', instruction_mode: str = '', suppress_user_message: bool = False):
        hidden_instruction = self.tavern_service._hidden_instruction_text(instruction_mode)
        prepared = self.tavern_service.prepare_generation(chat_id, text=text, preset_id=preset_id, hidden_instruction=hidden_instruction)
        provider = get_tavern_provider(provider_id or prepared['providerId'])
        client = TavernModelClient(self.tavern_service.merge_generation_provider(provider, prepared['preset']))
